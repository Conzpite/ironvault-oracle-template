...
def expected_rows_is_more_than_range(expected_rows, range):
    return expected_rows > range;
...
def generate_table_values(dice_num, dice_sides, expected_rows, dataset):
    ...
    if dice_num == 1:
        if expected_rows != 0:
            ...
            if expected_rows_is_more_than_range(expected_rows, dice_sides):
        ...
        dice_range = max_roll_value - min_roll_value + 1;

        if expected_rows != 0:
            # Calculate values based on expected rows
            row_range = dice_range // expected_rows; # How much range each row should cover
            remainder = dice_range % expected_rows; # How many rows should get the extra leftovers

            if expected_rows_is_more_than_range(expected_rows, dice_range):
                print("More expected rows requested than is possible for dice of {}d{}".format(dice_num, dice_sides));
                print("Changing to {} rows".format(dice_range));
                expected_rows = dice_range;
                row_range = 1;
                remainder = 0;

            dice_value = min_roll_value - 1;
...
def write_to_file(header, body, fileName, filePath='output/', always_overwrite=False):
    ...
    try:
        ...
        exit("Unable to write to {}".format(full_path))
...
def read_dataset_from_file(filepath, delimiter, auto_trim):
    ...
    try:
        ...
        exit("File not found. Check the path variable and filename")
...
def create_default_filename(dice_expression, expected_rows):
    ...
    dice_range = dice_sides if dice_num == 1 else (dice_num * dice_sides - dice_num + 1)

    if expected_rows_is_more_than_range(expected_rows, dice_range):
        # rows will be decreased to meet sides in this scenerio, so no need to append rows
        pass;
    elif expected_rows != 0:
        filename += "_{}".format(expected_rows);

    return filename;

def validateFileName(output_name):
    if output_name[0].isdigit() :
        # Need to prevent output file from starting with a digit
        exit("Output file must not begin with a number. Please change the output file name and try again.");
...
def main():
    ...
    parser.add_argument("-r", "--rows", nargs="?", default=0, const=0, help="Expected number of results. A non-zero input value here, along with a dice experssion of dY, will result in dice values being consolidated in that many rows (a d6 with rows of 3 result in 1-2, 3-4, 5-6 columns). In the event that that the values cannot be evenly split, the extra values will be spread among the rows, starting from the first row", type=int)

    # Output file related
    parser.add_argument("-o", "--output-file_name", nargs="?", default="", const="", help="Name of the generated file (excluding .md file extension). If left blank/not used, will default to a generated name instead", type=str)
